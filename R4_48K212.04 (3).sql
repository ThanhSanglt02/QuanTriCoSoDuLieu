    ...
    WHILE @i <= 1000
		BEGIN
			...
			SET @sdtncc =	CASE   --Rand() nó sẽ cho ra kiểu dữ liệu là float
								WHEN floor(RAND(CHECKSUM(newid())) * 3) = 0 THEN '09'   --checksum(newid()): hạn chế trùng lặp cho kết qủa ngẫu nhiên
								WHEN floor(RAND(CHECKSUM(newid())) * 3) = 1 THEN '01'	--floor sẽ cho ra kết quả là số có mẫu sau: 1.0, 2.0, 3.0, 10.0
    ...
    WHILE @i <= 1000
		BEGIN
			...
			VALUES (
				...
				DATEADD(DAY, -FLOOR(RAND() * 1825), GETDATE()),  -- ngày hiện tại sẽ được trừ đi trong hàm rand từ 0 -> 1825
				(CASE FLOOR(RAND() * 3)
					WHEN 0 THEN '09'
					WHEN 1 THEN '01'
					WHEN 2 THEN '07'
...
BEGIN
    ...
    BEGIN
        ...
        IF RAND() < 0.5  -- rand() trả về giá trị ngẫu nhiên kiểu float từ 0 đến 1
			BEGIN
        ...
        VALUES (
            'HDN'+RIGHT('00000' + CAST(@i AS VARCHAR(4)), 5), @NgayDat, @GhiChu, @MaNCC,@MaNV, @MaDVVC)
...
BEGIN
    ...
    WHILE @i <= 1000
		BEGIN
			...
			Set @DonGiaNhap = CAST(800 * RAND() AS DECIMAL(10, 2))  --lấy trừ 0 -> 800. Vì là decimal(10,2) --> xuất hiện thêm 2 số sau dấu . 
...
select CAST((520.53 * 10) % 10 AS INT)
select (520.53 * 10) % 10  --5205.30 % 10 
...
AS
BEGIN
    DECLARE @i INT = 1, @MaHDN varCHAR(20),@MaHangVin varCHAR(10),@SL_Nhap INT
    WHILE @i <= 2000
		BEGIN
			...
			SET @SL_Nhap = FLOOR(RAND() * 1000) + 1 -- vi luong khong duoc <= 0 và hàm rand() chạy từ 0 -> 1000
...
BEGIN
    ...
    WHILE @i <= 1000
		BEGIN
			...
			-- Tạo loai khach hang
			SET @loaiKH = CAST(ABS(CHECKSUM(NEWID())) % 2 AS INT) --cái này sử dụng hàm rand() được 
			...
			If @@ROWCOUNT=0
				begin 
					set @i=@i  --nếu xảy ra lỗi khi insert thì gán lại i để không bị thâm hụt dữ liệu
					Continue
				End
...
begin
    ...
    WHILE @i <= 1000
		BEGIN
			...
			-- Datediff(second,@startDate, @EndDate): lấy số giấy từ năm hiênj tại đến số giây của năm 2020 
			SET @ngayban = DATEADD(SECOND,FLOOR(RAND() * DATEDIFF(SECOND, @StartDate, @EndDate)), @StartDate)  --datediff: lấy sau trừ trước
...
begin
	...
	while @i <= 2000
		begin
			...
			select @sLBan = FLOOR(RAND() * 20) + 1 --phaỉ thêm điều kiện số lượng bán < sl sản phẩm
...
--proc update nha cung cap
go
create or alter proc spUpdateNCC	@maNCC varchar(20),
									@tenncc Nvarchar(40),
									@diachiNCC Nvarchar(50),
									@sdt_ncc varchar(15),
									@kq bit out
...
-- kiem soat update 
go
create or alter proc sp_updateDat	@maHDN varchar(30), 
									@maNCC varchar(30), 
									@maNV varchar(30), 
									@maDVVC varchar(30), 
									@ngayDat datetime,
...
as
begin
	declare @newID varchar(15), @oldID varchar(15), @maxId varchar(20)

	select @maxId = max(MaHDN) from DAT  --MHV 00001
	select @newID = right(@maxId,5) + 1
	select @newID = 'HDN' + right('00000' + @newID , 5)
	return @newID
end
...
-----update
go
create or alter proc spUpdateSP	@maHV varchar(20),
								@maSP varchar(20),
								@tensp Nvarchar(40),
								@DVT Nvarchar(20),
								@DonGiaNhap decimal(10,2),
...
begin
	...
	begin
		...
		set TenSP = @tensp, 
			...
			HanSuDung = @hanSuDung,
			Sl_SP = @sl_sp
		where MaHangVin = @maHV and MaSP = @maSP 
...
--tong tien hoa don ban
go
create function getTotalHDB(@maHDB varchar(15))
...
begin
	declare @soLuongBan int,
			...
			@rowCount int,
			@index int;
		select @rowCount = count(*) from BAN_CHITIET where MaHDB = @maHDB
		set @index = 1
...
select * from DAT_CHITIET
where MaHangVin = 'MHV00693'
select * from SANPHAM
where MaHangVin = 'MHV00693'


--tong tien hoa don nhap
go
create or alter function getTotalHDN(@maHD varchar(20))
returns numeric(20,2)
as
begin
	...
	select @rowCount = count(*) from DAT_CHITIET where MaHDN = @maHD	
	set @index = 1
	while @index <= @rowCount
	begin
		...
		if @khuyenmai <> 0
		begin
			set @tongTien = @tongTien + (@giaNhap * @soLuongNhap * @khuyenmai) 
		end
		else 
		begin
...
if left(@maHD,3) = 'HDB'
begin
	select ban_chitiet.mahdb,sanpham.mahangvin,dongiaban, sl_ban, (dongiaban * sl_ban) as thanhtien, khuyenmai, uudai,dbo.getTotalHDB(@mahd) as TongTien
	from Ban join BAN_CHITIET on Ban.MaHDB = BAN_CHITIET.MaHDB join SANPHAM on BAN_CHITIET.MaHangVin = SANPHAM.MaHangVin
			join KHACHHANG on KhachHang.maKH = BAN.MaKH
...
begin
	select  dat_chitiet.mahdn,sanpham.mahangvin, dongianhap, sl_nhap,(dongianhap * sl_nhap) as thanhtien, khuyenmai,dbo.getTotalHDN(@mahd) as TongTien
	FROM DAT join DAT_chitiet on DAT.MaHDN = DAT_chitiet.MaHDN join SANPHAM on DAT_chitiet.MaHangVin = SANPHAM.MaHangVin
	where dat_CHITIET.MaHDn = @mahd
end
...
--go
--create or alter proc datchitiet_capnhatsoluongsp(@mahdn varchar(20), @mahangvin varchar(20), @soluongnhapmoi int, @ret nvarchar(40)out)
--As
--begin
...
--			Return
--		End
--	select @soluongnhapcu=Sl_Nhap from DAT_CHITIET where MaHangVin=@mahangvin
--	Update SANPHAM
...
select * from DAT_CHITIET
select Sl_Nhap from DAT_CHITIET where MaHangVin='MHV00298'
go
declare @soluongnhapcu int
select @soluongnhapcu=Sl_Nhap from DAT_CHITIET where MaHangVin='MHV00298'
print(@soluongnhapcu)
select * from SANPHAM 
where MaHangVin='MHV00298'
...
print @check
select * from SANPHAM --/MHV00008  //65 - 30 = 35



-- khi cập nhật lại số lượng nhập mới, thì slsp cũng phải thay đổi bằng slsp = slsp - slnhap cũ + slnhap mới
go
create or alter trigger datchitiet_capnhatso
on Dat_ChiTiet
after Update
as
begin
	declare @soluongnhapcu int, @soluongnhapmoi int, @mahangvin varchar(20), @maHDN varchar(20), @ret int;

	select @mahangvin = MaHangVin, @maHDN = MaHDN, @soluongnhapmoi = Sl_nhap from inserted 
...
update DAT_CHITIET
set Sl_Nhap = 300
where MaHDN = 'HDN0001' and MaHangVin = 'MHV00298'
select * from DAT_CHITIET
where MaHDN = 'HDN0001'
select * from SANPHAM 
where MaHangVin = 'MHV00298'--MHV00298 , 797
------------------------------END THỦ TỤC-----------------------------------
...
select * from ban_chitiet
where mahangvin = 'MHV00433'

/*---------------------------------------------------ACCOUNT--------------------------------------------*/
-- Kiểm tra tài khoản và mật khẩu va Phòng chống sql injecttion 
go
create or alter proc spPreventSqlInjection	@tenTk varchar(max), 
											@MatKhau varchar(max),
											@tb int out
as 
begin
    ...
    BEGIN TRY
		set @tb = 1
		if not exists (select 1 from ACCOUNT where TenTK = @tenTk and CAST(DECRYPTBYKEY([encrypted_matkhau]) AS NVARCHAR(15)) = @MatKhau )
		begin
			set @tb = 0
			return
		end

		-- Kiểm tra ký tự đặc biệt để phòng tránh SQL Injection
		IF @tenTk LIKE '%--%' OR @tenTk LIKE '%;%' OR @tenTk LIKE '%/*%' OR @tenTk LIKE '%*/%' 
		BEGIN
			SET @tb = 0;
			RETURN;
		END

		IF @MatKhau LIKE '%--%' OR @MatKhau LIKE '%;%' OR @MatKhau LIKE '%/*%' OR @MatKhau LIKE '%*/%'
		BEGIN
			SET @tb = 0;
			RETURN;
		END
...
declare @tb int
exec spPreventSqlInjection 'ThanhSang', '123', @tb out
print @tb
select * from account 
select * from Khachhang
